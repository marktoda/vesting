...
contract ChunkedVestingVaultFactory is IVestingVaultFactory {
        ...
        returns (address)
    {
        ...
        bytes memory data = abi.encodePacked(
            token, beneficiary, amounts.length, amounts, timestamps
        );
        ChunkedVestingVault clone =
            ChunkedVestingVault(implementation.clone(data));

        uint256 totalTokens = clone.vestedOn(type(uint256).max);
        IERC20Upgradeable(token).safeTransferFrom(
            msg.sender, address(this), totalTokens
        );
